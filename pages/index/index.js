...
Page({
  ...
  bindconfirm: function (e) {
    ...
    if (illegalChar.test(e.detail.value)) {
      wx.showModal({
        title: '您输入的内容中含有非法字符',
        content: '请您重新输入',
        complete: (res) => {
      ...
      if (1) {
        wx.request({
          url: 'https://zhuiyuan.origami.wang/medicine/fuzzySelectMedicineByMedicineName/' + e.detail.value+'?pageNum&pageSize=6' ,
          method: 'GET',
          header: {
        ...
        wx.request({
          url: 'https://zhuiyuan.origami.wang/medicine/selectMedicinesByComponentName/' + e.detail.value + '?pageNum&pageSize=6',
          method: 'POST',
          header: {
            'Content-Type': 'application/json'
          },
